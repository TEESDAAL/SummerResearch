...
def eaSimple(population, toolbox, cxpb, mutpb, elitpb, ngen, stats=None,
    ...
    def evaluate_individual(individual, weights: ndarray) -> tuple[float, np.ndarray]:
        ...
        errors = np.array(list(toolbox.map(
            e,
    ...
    total_errors = np.zeros(len(weights))
    for ind, (fit, errors) in zip(population, fitnesses_errors):
    ...
    for gen in range(1, ngen + 1):
        ...
        fitnesses_errors = [evaluate_individual(p, weights) for p in offspring]
        for ind, (fit, errors) in zip(offspring, fitnesses_errors):
            ind.fitness.values = (fit,)
            total_errors += errors
