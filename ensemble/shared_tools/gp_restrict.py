...
######################################
def genFull(pset, min_, max_, type_=None, exit_early=False):
    ...
    def condition(height, depth):
        ...
        return depth == height
    #print('it works', pset)
    return generate(pset, min_, max_, condition, type_, exit_early)

def genGrow(pset, min_, max_, type_=None, exit_early=False):
...
def genHalfAndHalf(pset, min_, max_, type_=None, exit_early=False):
    ...
    #print(method)
    return method(pset, min_, max_, type_, exit_early)

def genRamped(pset, min_, max_, type_=None, exit_early=False):
    """
    .. deprecated:: 1.0
    ...
    warnings.warn("gp.genRamped has been renamed. Use genHalfAndHalf instead.",
                  FutureWarning)
    return genHalfAndHalf(pset, min_, max_, type_, exit_early)

def generate(pset, min_, max_, condition, type_=__type__, exit_early=False):
    ...
    while len(stack) != 0:
        if exit_early and len(expr) > max_:
            return None
...
def genHalfAndHalfMD(pset, min_, max_, type_=None):
    expr=genHalfAndHalf(pset, min_, max_, type_=None, exit_early=True)
    #print('expr before', len(expr))
    while expr is None or len(expr)>max_:
        expr=genHalfAndHalf(pset, min_, max_, type_=None, exit_early=True)
    return expr

def genFullMD(pset, min_, max_, type_=None):

    expr=genFull(pset, min_, max_, type_=None, exit_early=True)
    #print('expr before', len(expr))
    while expr is None or len(expr)>max_:
        expr=genFull(pset, min_, max_, type_=None, exit_early=True)
        #print('expr before', len(expr))
