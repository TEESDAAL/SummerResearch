...
import numpy, numpy.typing as npt
...
type image = npt.NDArray[numpy.floating]
type vector = npt.NDArray[numpy.floating]
...
def all_lbp(image):
    #uniform_LBP
    lbp=local_binary_pattern((image*255).astype(numpy.uint8), P=8, R=1.5, method='nri_uniform')
    n_bins = 59
    hist, _ =numpy.histogram(lbp,n_bins, (0,59))
    return hist
...
def gab(left,the,fre):
    ...
    freq = fmax/(a**fre)
    thea = numpy.pi*the/8
    filt_real, _ = numpy.asarray(gabor(left,theta=thea,frequency=freq))
    return filt_real
...
def lbp(image):
    ...
    try:
        lbp = local_binary_pattern((image*255).astype(numpy.uint8), 8, 1.5, method='nri_uniform')
        lbp = numpy.divide(lbp,59)
    except: lbp = image
    return lbp
...
def compose[U, R](func: Callable[..., R], *args: Callable[[U], Any]) -> Callable[[U], R]:
    """Examples:
        - compose(f1, f2) -> lambda x: f1(f2(x))
        - compose(f1, f2, a, b) -> lambda x: f1(f2(x), a, b)
    """
    return lambda x: func(*[f(x) if callable(f) else f for f in args])


def starting_point[T]() -> Callable[[T], T]:
    return lambda x: x
